...
class MeasureService
  def measure_ten_columns_by_ips(limit)
    ...
    Benchmark.ips do |x|
      x.report("all") do
        ten_columns = TenColumn.limit(limit)
      ...
      x.report("select") do
        ten_columns = TenColumn.limit(limit).select(:item1, :item2)
        ten_columns.each do |model|
          "#{model.item1} #{model.item2}"
        end
      end
      ...
      x.report("pluck") do
        ten_columns = TenColumn.limit(limit).pluck(:item1, :item2)
        ten_columns.each do |model|
          "#{model[0]} #{model[1]}"
        end
      end
  ...
  def measure_thirty_columns_by_ips(limit)
    Benchmark.ips do |x|
      x.report("all") do
        ten_columns = ThirtyColumn.limit(limit)
        ten_columns.each do |model|
          "#{model.item1} #{model.item2}"
        end
      end
